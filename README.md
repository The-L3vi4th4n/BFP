# BFP
A brainf*ck inspired esolang which is designed to mess with you head (and for me to make a language which is complex but codeable for a beginer). The interpreter is programmed pretty poorly, which might affect performance but I have not tested that as that is not the purpose of it (which is to mess with the user's head). BFPH is also there just for the funny and it is meant to be unprogramable(without a lot of time and effort on your hands).
...
1) Multiply
Multiply is used to pass on a number to the symbols below. It is weird in the fact that it starts out as 1 and adds every number individually so:
0  -->  is 1
1  -->  is 2
2  -->  is 3
4  -->  5
...
11  -->  3
12 --> 4
As you may have noticed, this causes it to be impossible to make multiply 0 so some symbols substract 1 from multiply. Multiply resets every non-number symbol.
